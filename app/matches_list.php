...
if ($isLogged == true) {
            ...
            if ($result->num_rows > 0) {
                ...
                while ($row = $result->fetch_assoc()) {
                    ...
                    echo "<tr>
                            ...
                            <td>
                                <form method='POST' action='remove_match.php'>
                                    <input type='hidden' name='match_id' value='" . $row["id"] . "'>
                                    <button type='submit' class='reset-button'>Usuń</button>
                                </form>
                                <form method='GET' action='edit_match.php'>
                                    <input type='hidden' name='match_id' value='" . $row["id"] . "'>
                                    <button type='submit' class='edit-button primary-button'>Edytuj mecz</button>
                                </form>
                            </td>
                        </tr>";
                }
    ...
    if ($resultAllRounds) {
        if ($resultAllRounds->num_rows > 0) {
            ...
            echo "<button class='primary-button pick-round-button' name='round' value='all'>Wszystkie mecze</button>";

            // Wyświetlanie przycisków dla poszczególnych kolejek
            while ($row = $resultAllRounds->fetch_assoc()) {
                echo "<button class='primary-button pick-round-button' name='round' value='" . $row["match_round"] . "'>Kolejka " . $row["match_round"] . "</button>";
            }
            echo "</form>";
            echo "</div>";
